...
inline ConfigManager createConfigManager() { return ConfigManager(NUM_POTS, NUM_BUTTONS); }

inline LEDManager createLEDManager() { return LEDManager(hwConfig); }
...
inline std::vector<EnvelopeFollower> createEnvelopeFollowers(PotentiometerManager *pm) {
    return {
        EnvelopeFollower(A0, pm, 0), EnvelopeFollower(A1, pm, 1), EnvelopeFollower(A2, pm, 2),
        EnvelopeFollower(A3, pm, 3), EnvelopeFollower(A6, pm, 4), EnvelopeFollower(A7, pm, 5),
    };
}
