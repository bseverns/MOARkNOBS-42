...
4. **Subsystem walkthroughs** – [`systemFlow/`](systemFlow/) is the guided tour with words, not just screenshots.

## Teaching move

Pick one signal and follow it twice: once in the image, once in [`systemFlow/hw/`](systemFlow/hw/). If the two stories disagree, the docs are stale and the board wins.
