...
1. **Run the gauntlet** – build (`pio run -d firmware -e teensy40_main`) and test (`pio test -d firmware -e teensy40_unity -vvv`, `npm --prefix bridge test`, `npm --prefix App test`). Don't ship broken noise.
