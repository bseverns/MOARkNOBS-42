...
| `GET_ARP`                                                       | –                           | read live arp state, including `pattern_length`                                                                                           |
| `SET_ARP` `<ticks>,<shape>,<swing>,<gate>,<octave>[,<pattern>]` | ints                        | update and persist arp state; optional pattern length is clamped to 2‑16                                                                  |
...
Send `SET_PROFILE <id>,<payload>` with newline-terminated JSON to stage a new snapshot. The parser accepts partial payloads: include only the keys you changed (`led`, `lfos`, `routes`, `slots`, `arp`, etc.), and the firmware will merge them into the current snapshot before persisting to EEPROM. The `arp.pattern_length` field accepts 2 through 16. A successful store returns `OK`, and when you `SET_PROFILE` the active slot the firmware immediately applies the snapshot (LEDs, routes, envelopes, and MIDI channels update without needing a reboot).
