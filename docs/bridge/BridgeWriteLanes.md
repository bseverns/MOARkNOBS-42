...
- The bridge session tracks `liveConfig`, `stagedConfig`, `dirty`, `deviceAuthority`, `draftState`, `clientApplyId`, `stagedRevision`, `stagedDigest`, and `lastApplyResult`.
- Preflight rejection happens before serial writer ownership and does not create uncertainty. Network/response loss after the Apply request begins is `transmission-unknown`; a correlated firmware error is `device-rejected-before-commit`.
