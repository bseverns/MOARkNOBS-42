...
If you need the conservative compatibility matrix first, read [Host Compatibility](../reference/HostCompatibility.md).
...
| Browser-driven bridge console                    | Node.js 22 plus a desktop browser                                                        | Verified in repo code/tests       | The bridge UI serves the configurator at `/app/` and exposes the firmware line protocol over `/ws`.                             |
