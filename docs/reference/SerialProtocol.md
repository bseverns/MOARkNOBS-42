...
| `GET_SCHEMA`        | `GET_SCHEMA`                                                  | JSON schema                                                                            | Returns `ConfigManager::makeSchema()`. The schema describes the compatibility-shaped `GET_CONFIG` export, including numeric slot `type`, `channel`, and `ef_index`. |
