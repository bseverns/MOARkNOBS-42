...
| ------------------------ | ----------------------------------------------------------------------------------------------------------------- | --------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Full battery             | `./test.sh`                                                                                                       | Host machine, plus Teensy 4.0 when HIL is available | Runs every native firmware seam, the main firmware build, JS suites, and docs checks; adds executing Unity/full-stack lanes when a board is present.                       |
| Native firmware seam     | `pio test -d firmware -e native_biquad -vvv`                                                                      | Host machine only                                   | Exercises Arduino-independent DSP behavior without a connected Teensy; this lane grows only through deliberate portable seams.                                           |
...
| Unity compile/link gate  | `pio test -d firmware -e teensy40_unity --without-uploading --without-testing`                                     | Host machine only                                   | Push/PR and release CI compile and link the complete custom Unity target without requiring attached hardware.                                                            |
| Native transport policy  | `pio test -d firmware -e native_transport -vvv`                                                                    | Host machine only                                   | Executes token reset/refill/capacity, clock-wrap, Note-priority, fairness, and failed-attempt policy on every push/PR and release preflight.                                |
| Native modulation policy | `pio test -d firmware -e native_modulation -vvv`                                                                   | Host machine only                                   | Executes EF/LFO composition order, every LFO mode, stage clamping, persisted-lane sanitization, and the exact legacy SlotValue replacement curve.                           |
| Native button gestures   | `${CXX:-c++} -std=c++17 -Wall -Wextra -Werror -I firmware/include tools/button_gesture_timing_test.cpp -o /tmp/mn42-button-gestures && /tmp/mn42-button-gestures` | Host machine only | Executes the production-shared exclusive-double classifier at threshold boundaries, Ctrl3/Ctrl4 repeated releases, chord consumption, release ordering, and tap-interval conversion. |
| Native button interpreter | `${CXX:-c++} -std=c++17 -Wall -Wextra -Werror -I firmware/include firmware/src/ButtonGestureInterpreter.cpp tools/button_gesture_interpreter_test.cpp -o /tmp/mn42-button-interpreter && /tmp/mn42-button-interpreter` | Host machine only | Gated by the local preflight, firmware doctor, and release workflows; exercises legacy and exclusive doubles, immediate Ctrl5 tap releases, long-confirm flow, every two-button order, all three-button rolling permutations, all 24×24 Config Mode press/release permutations, all-up sub-combo re-arming, and short/long special chords. |
| Native persistence seams | `pio test -d firmware -e native_persistence -vvv`                                                                  | Host machine only                                   | Executes storage-region adjacency, schema-6/7 migration address calculations, modulation payload sanitization/packing, and semantic CRC coverage.                          |
| App seam regressions     | `npm --prefix App test`                                                                                           | Host machine only                                   | Proves Scope timeline geometry, deterministic simulator LFO/EF rehearsal differences, live-control draft protection, arp authority UI, native transport adaptation, and browser-local metadata without hardware. Simulator results are not physical-device evidence. |
...
| Persistence abuse       | `node firmware/system_test/mn42_persistence_abuse_runner.js` or `pio run -d firmware -e teensy40_eeprom_persistence -t upload` | Save/load, reboot, backup recovery, and manual fault-injection prep | JSON report + serial log      |
...
| Persistent diagnostics record and `GET_DIAGNOSTICS` shape                                     | Covered                  | Unity                                   | Event-driven boot/load/apply/error/high-water updates are persisted through the storage backend when space is available, and the host-facing diagnostics payload shape is asserted directly.                                                                     |
...
| Runtime orchestration                                                                         | Covered                  | Unity                                   | Pending note-offs and diagnostics are asserted directly. Transport tests cover a 42-slot CC burst after long simulated setup, sustained CC coalescing/fairness, sustained changing-value traffic across 42 Notes, Note On/Off balance and queue drain, maximum pending depth, queue-full retry without a duplicate success, and mixed CC/Note/NRPN/SysEx admission across all slots.                      |
...
| Browser configurator transport + capability gating                                            | Covered                  | App Playwright                          | Simulator UX, tuning-catalog/schema parity, deterministic subtree recipes, musician-first/technical-second labels, device-patch draft parity, staged pre-Apply return, native `HELLO/GET_*/SET_ALL` adaptation, staged/live/browser-only boundaries, capability gating, and browser-local metadata are covered without hardware. |
...
| Primary/backup/default config recovery and interrupted profile-save ordering                  | Covered                  | Unity + persistence abuse wrapper       | Unity covers primary-good/backup-bad, primary-bad/backup-good, both-bad/defaults, and interrupted profile-save recovery. The persistence abuse wrapper records those lanes as Unity evidence and keeps attached-board corruption injection manual for now.       |
...
- black-box diagnostics record updates and `GET_DIAGNOSTICS` payload shape
