...
| Native button interpreter | `${CXX:-c++} -std=c++17 -Wall -Wextra -Werror -I firmware/include firmware/src/ButtonGestureInterpreter.cpp tools/button_gesture_interpreter_test.cpp -o /tmp/mn42-button-interpreter && /tmp/mn42-button-interpreter` | Host machine only | Executes the pure semantic event machine across legacy and exclusive doubles, long-confirm flow, chord settling and consumption, plus short/long special-chord classification. |
