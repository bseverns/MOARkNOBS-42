...
| Browser configurator transport + capability gating                                            | Covered                  | App Playwright                          | Simulator UX, native `HELLO/GET_*/SET_ALL` adaptation, device-backed profile/macro/scene command mapping, staged/live/browser-only state-boundary protection, older-firmware capability gating, and browser-local slot metadata are covered without hardware.    |
