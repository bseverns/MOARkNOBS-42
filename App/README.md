...
- Stage envelope meters report firmware active/idle state and configured route counts. Selected-slot status keeps the last reported output while showing current EF/LFO source values. The clock summary identifies internal/external source plus the applicable BPM and explicitly reports internal fallback while external follow is armed but its signal is missing.
