...
- **Simulator Controls** – Configure's empty-state **Start simulator** action swaps WebSerial for the canned transport and connects in one step. Lab retains the explicit transport toggle. Because the simulator obeys the same throttled paint loop documented in [Runtime Contract](#runtime-contract), you can chase layout timing bugs or automation macros without a Teensy on the desk. Its EF telemetry uses deterministic, smoothly varying rehearsal signals so scope screenshots demonstrate readable motion instead of random noise.
