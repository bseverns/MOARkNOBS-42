...
- **Simulator Controls** – Configure's empty-state **Start simulator** action swaps WebSerial for the canned transport and connects in one step. Lab retains the explicit transport toggle. Because the simulator obeys the same throttled paint loop documented in [Runtime Contract](#runtime-contract), you can chase layout timing bugs or automation macros without a Teensy on the desk. Its deterministic LFO traces follow the declared Sine/Triangle/Saw/Square/Sample & Hold/Random Slew shape. EF Scope traces and their activity status remain synthetic source-level rehearsal, independent of slot ordering and slot tuning; each slot's resolved output then applies that slot's illustrative tuning response. **Simulator modulation is behavioral rehearsal, not analog calibration or hardware-validation evidence.**
...
- **Live-control drafts** – Multi-field Jitter and Device Clock edits keep a local unsent draft while the musician moves between controls. Telemetry continues elsewhere, but it cannot rewrite those pending fields until **Push live override** succeeds or the connection resets. This remains separate from the main staged configuration Apply lane.
...
- The Lab Scope keeps a separate rolling history for every manifest-advertised envelope follower. **EF traces: Active** uses firmware `efStatus[]` plus a 1.5-second visual hold so transient movement fades instead of disappearing; **All** reveals every EF trace. LFOs remain visible in both modes as secondary dashed traces. The 2s/5s/10s Window control changes the host-observed elapsed-time X axis without clearing history. In structured Bridge mode that timestamp is observed when the Bridge receives telemetry; other transports use App receipt time. It is visualization timing, not firmware-source or latency-grade timing; bench latency tools remain authoritative. The persistent view line names Active/All and any temporary EF focus, while **Leave solo** returns to the underlying view. Inactive histories continue recording, and the no-active-EF state says so explicitly.
- Scope X time is host-observed telemetry arrival time: it measures the spacing between App callbacks after browser, WebSerial, Bridge, and frame-assembly behavior. Timeline calculations use the same receipt-time clock as stored samples and EF activity expiry; requestAnimationFrame time is used only for render pacing and FPS. It is intended for musical visualization, not source-device or round-trip latency measurement. Use the [bench latency method](../docs/bench/latency/method.md) for latency-grade evidence.
...
- Lab presents the arpeggiator as one **Shared Arp Engine** with an explicit Live Engine / Profile Defaults authority switch. Selected Slot owns root, assignment, and start/stop context; the profile-wide assignment list remains available as a bulk disclosure. No firmware ownership changed.
